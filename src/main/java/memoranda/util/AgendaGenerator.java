...
import memoranda.interfaces.ITaskList;
...
public class AgendaGenerator {
	...
	private static String expandRecursively(IProject p, CalendarDate date, ITaskList tl, ITask t, Collection expandedTasks, int level) {
		...
		for (Iterator iter = st.iterator(); iter.hasNext();) {
			ITask subITask = (ITask) iter.next();
			//			if(Context.get("SHOW_ACTIVE_TASKS_ONLY").equals(new Boolean(true))) {
			//                if (!((subITask.getStatus() == ITask.ACTIVE) || (subITask.getStatus() == ITask.DEADLINE) || (subITask.getStatus() == ITask.FAILED))) {
			//                	continue;
			//            	}	
			//			}
			s = s + renderTask(p,date,tl, subITask,level,expandedTasks);
			if (expandedTasks.contains(subITask.getID())) {
				s = s + expandRecursively(p,date,tl, subITask,expandedTasks,level + 1);
			}
		}
		s += "\n</ul>\n";
